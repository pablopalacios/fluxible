...
const createComponent = (context) => {
    ...
    const WrappedComponent = provideContext(
        connectToStores(DumbComponent, stores, getStateFromStores)
    );

    const renderer = TestRenderer.create(
        <WrappedComponent spy={spy} context={context.getComponentContext()} />
    );

    const root = renderer.root;
    const component = root.findByType(DumbComponent).instance;
